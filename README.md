...
- The prediction uncertainty shows a slight increase when increasing the number of sampling runs from 5 to 10. Considering that the uncertainty values lie within the range [0, 1.922), this slight variation is reasonable, as a larger number of samples is more likely to introduce additional response diversity, especially under image corruptions. Moreover, **the ranking of LVLMs in terms of both prediction performance and uncertainty remains highly consistent with that reported in our main paper**. For instance, InternVL-3-8B still achieves the best performance with the lowest uncertainty, while Deepseek-VL2-3B remains the worst-performing model. These results support the reliability of our experimental findings.
