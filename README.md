...
- From Table X, it can be observed that under 10 sampling runs, the average prediction accuracy of LVLMs remains almost identical to that under 5 sampling runs. The prediction uncertainty shows a slight increase when increasing the number of sampling runs from 5 to 10. Considering that the uncertainty values lie within the range [0, 1.922), the average increase of 7.2% is reasonable, as a larger number of samples is more likely to introduce additional response diversity, especially under image corruptions.
